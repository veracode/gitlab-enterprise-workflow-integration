...
async function veracodeScan() {
    ...
    if (executePipeline) {
        ...
        await pipelineScan(apiId, appKey, appProfileName, filterMitigatedFlaws, breakBuildOnFinding, breakBuildOnError, userErrorMessage, policyName, breakBuildOnInvalidPolicy, createIssue, debug, commitSha);
    }
    if (executeSandbox) {
    ...
    if (executeSca) {
        ...
        await scaScan(sourceRepoCloneUrl, scaAgenToken, scaUrl, sourceBranch, breakBuildOnFinding, breakBuildOnError, userErrorMessage, createIssue, debug, commitSha);
    }
    if (executeIac) {
        console.log(`Executing iac scan on ${projectName} repo for ${sourceBranch} branch`);
        await iacScan(sourceBranch, breakBuildOnFinding, breakBuildOnError, userErrorMessage, debug, commitSha)
    }
}
veracodeScan();
